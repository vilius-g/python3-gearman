...
from python3_gearman.constants import PRIORITY_NONE, PRIORITY_LOW, \
    PRIORITY_HIGH
from python3_gearman.errors import ProtocolError
...
GEARMAN_PARAMS_FOR_COMMAND = {
    ...
    # Gearman commands 20-29
    GEARMAN_COMMAND_STATUS_RES: ['job_handle', 'known', 'running',
...
def pack_text_command(cmd_type, cmd_args):
    ...
    if cmd_type != GEARMAN_COMMAND_TEXT_COMMAND:
        raise ProtocolError(
            'Unknown cmd_type: Received %s, expecting %s' %
            (get_command_name(cmd_type),
             get_command_name(GEARMAN_COMMAND_TEXT_COMMAND)))

    cmd_line = cmd_args.get('raw_text')
    if cmd_line is None:
        raise ProtocolError(
            'Did not receive arguments any valid arguments: %s' %
            cmd_args)
